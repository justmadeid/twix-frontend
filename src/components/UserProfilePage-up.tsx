...
export const UserProfilePage: React.FC<UserProfilePageProps> = ({ user, onBack }) => {
  ...
  const handleFollowersComplete = (result: any) => {
    ...
    console.log('Raw followers data sample:', result?.users?.[0] || result?.[0]);
    let users = [];
    if (result?.users) {
      users = result.users;
    ...
    const mappedFollowers = users.map((follower: any) => {
      console.log('Mapping follower:', follower);
      return {
        ...
        profile_image_url: follower.profile_image_url || follower.avatar || follower.profile_image || follower.image_url,
  ...
  const handleFollowingComplete = (result: any) => {
    ...
    console.log('Raw following data sample:', result?.users?.[0] || result?.[0]);
    let users = [];
    if (result?.users) {
      users = result.users;
    } else if (Array.isArray(result)) {
    ...
    const mappedFollowing = users.map((followingUser: any) => {
      console.log('Mapping following user:', followingUser);
      return {
        ...
        profile_image_url: followingUser.profile_image_url || followingUser.avatar || followingUser.profile_image || followingUser.image_url,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden relative">
          {/* Back to Search Button - Upper Right Corner */}
          <button
            onClick={onBack}
            className="absolute top-4 right-4 z-20 flex items-center space-x-2 px-4 py-2 bg-white/90 backdrop-blur-sm text-gray-700 hover:text-gray-900 hover:bg-white rounded-lg shadow-md transition-all duration-200 border border-gray-200/50 hover:shadow-lg"
          >
