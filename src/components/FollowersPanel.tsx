...
import { TwitterFollow } from '../types/api';
import TaskMonitor from './TaskMonitor';
import { UserPlus, UserMinus, User, Heart, MessageCircle, Calendar, MapPin, BadgeCheck } from 'lucide-react';
...
export const FollowersPanel: React.FC = () => {
  ...
  const [users, setUsers] = useState<TwitterFollow[]>([]);
  ...
  const getBentoSize = (index: number, total: number, user: TwitterFollow) => {
    const hasExtensiveInfo = !!(user.bio && user.location);
    const isHighFollowerCount = (user.followers || 0) > 10000;
    const isVerified = user.blue_verified;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Auto-fetchesfollowers  by default • Switch tabs to fetch following
              </p>
            </div>
            {/* User count display */}
            {users.length > 0 && (
              ...
              <div className="relative">
                ...
                <div className="space-y-2">
                  <button
                    ...
                    disabled={isLoading}
                    className={`w-full py-3 px-4 text-sm font-semibold rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${activeTab === 'followers'
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className={`w-full py-3 px-4 text-sm font-semibold rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${activeTab === 'following'
                        ? 'bg-purple-500 text-white shadow-lg transform scale-[1.02]'
                        : 'bg-white/60 text-gray-600 hover:bg-white/80 hover:text-purple-500'
                      }`}
                  >
            ...
            <div className="group relative overflow-hidden bg-gradient-to-br from-blue-50/80 to-cyan-50/80 backdrop-blur-sm rounded-2xl border border-blue-100/50 p-6 hover:shadow-lg hover:scale-105 transition-all duration-300">
              ...
              <div className="relative">
                ...
                <p className="text-2xl font-bold text-blue-700 mb-1">
                  {users.filter(user => user.blue_verified).length}
                </p>
            ...
            <div className="group relative overflow-hidden bg-gradient-to-br from-emerald-50/80 to-teal-50/80 backdrop-blur-sm rounded-2xl border border-emerald-100/50 p-6 hover:shadow-lg hover:scale-105 transition-all duration-300">
              ...
              <div className="relative">
                ...
                <p className="text-2xl font-bold text-emerald-700 mb-1">
                  {users.length > 0
                    ? formatNumber(Math.round(users.reduce((sum, user) => sum + (user.followers || 0), 0) / users.length))
                    : '0'
                  }
                </p>
        ...
        {users.length > 0 && (
          <div className="bg-gradient-to-br from-slate-50/80 to-gray-50/80 backdrop-blur-sm rounded-2xl border border-slate-100/50 p-6 hover:shadow-lg transition-all duration-300">
            ...
            <div className="grid grid-fit-content grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-[800px] overflow-y-auto custom-scrollbar pr-3">
              {users.map((user, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start space-x-3 mb-3">
                      <div className="flex-shrink-0">
                        {user.profile_image_url ? (
                          ...
                          <div className={`bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl flex items-center justify-center border-2 border-white shadow-sm group-hover:border-[#0dbcf9] transition-all duration-300 ${isLargeCard ? 'w-16 h-16' : 'w-12 h-12'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center">
                          <h4 className={`font-semibold text-gray-900 truncate group-hover:text-[#0dbcf9] transition-colors duration-300 ${isLargeCard ? 'text-base' : 'text-sm'}`}>
                            {user.name}
                          </h4>
                          {user.blue_verified && (
                            // <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-[#0dbcf9] text-white rounded shadow-sm">
                            //   ✓ Verified
                            // </span>
                            <div className="p-2">
                              <BadgeCheck className="w-4 h-4 text-[#0dbcf9]" />
                            </div>
                          )}
                        </div>
                        <span className={`text-gray-500 truncate group-hover:text-[#0dbcf9] transition-colors duration-300 ${isLargeCard ? 'text-sm' : 'text-xs'}`}>
                          @{user.username}
                        </span>
                      </div>
                    </div>
                    ...
                    {user.bio && (
                      <div className="mb-3">
                        <p className={`text-gray-700 leading-relaxed group-hover:text-gray-800 transition-colors duration-300 ${isLargeCard ? 'text-sm' : 'text-xs'
